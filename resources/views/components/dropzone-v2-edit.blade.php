    ...
    document.addEventListener("DOMContentLoaded", () => {
        Dropzone.options[_.camelCase("{{ $attributes['id'] }}")] = {
            ...
            success: function(file, response) {
                @this.addMedia(response.media , "{{ $attributes['in'] ?? 'default' }}")
            },
            removedfile: function(file) {
                ...
                if (file.xhr) {
                    ...
                    @this.removeMedia(response.media , "{{ $attributes['in'] ?? 'default' }}")
                } else {
                    @this.removeMedia(file , "{{ $attributes['in'] ?? 'default' }}")
            ...
            init: function() {
                document.addEventListener("livewire:load", () => {
                    let files = @this.mediaCollections[
                        "{{ $attributes['collection-name'] ?? 'default' }}"]["{{ $attributes['in'] }}"]
