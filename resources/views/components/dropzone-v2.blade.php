<div wire:ignore>
    <div id="multi_dropzone_{{ $attributes['in'] }}"></div>
</div>

{{-- @push('scripts') --}}
<script>
    window.addEventListener("reApplyDropzone_{{ $attributes['in'] }}", event => {
        function initializeDropzone(elementId) {
            ...
            new Dropzone('#' + elementId, {
                ...
                init: function() {
                    document.addEventListener("livewire:load", () => {
                        let files = @this.mediaCollections[
                            "{{ $attributes['collection-name'] ?? 'default' }}"
                        ]
                        if (files !== undefined && files.length) {
                            files.forEach(file => {
                                let fileClone = JSON.parse(JSON
                                    .stringify(
                                        file))
                                this.files.push(fileClone)
                                this.emit("addedfile", fileClone)

                                if (fileClone.preview_thumbnail !==
                                    undefined) {
                                    this.emit("thumbnail", fileClone,
                                        fileClone
                                        .preview_thumbnail)
                                } else {
                                    this.emit("thumbnail", fileClone,
                ...
                error: function(file, response) {
                    ...
                    let message = $.type(response) === 'string' ? response : response
                        .errors
                        .file
                    return _.map(file.previewElement.querySelectorAll(
        ...
        var newDropzoneId = "dropzone_{{ $attributes['in'] }}";
        if (!document.getElementById(newDropzoneId)) {
            var newDropzoneElement = $('<div class="dropzone" id="' + newDropzoneId + '"></div>');

            // Agregar el nuevo elemento al contenedor
            $("#multi_dropzone_{{ $attributes['in'] }}").append(newDropzoneElement);
            // Llamar a la función para inicializar Dropzone en el nuevo elemento
    ...
    //     //                         this.emit("thumbnail", fileClone, fileClone
    //     //                             .preview_thumbnail)
    //     //                     } else {
    ...
    //     //         return _.map(file.previewElement.querySelectorAll('[data-dz-errormessage]'), r => r
    //     //             .textContent = message)
    //     //     }
    //     // }
    // })
